...
import { isAllowedForReview, isOptedOutAuthor } from './config';
...
import { findingIdOf, type PullRequestSummary } from './review';
...
function selectCandidates(activePullRequests: readonly PullRequestSummary[]): readonly PullRequestSummary[] {
  return [...activePullRequests]
    ...
    .filter((pullRequest) => !isOptedOutAuthor(pullRequest.authorName, pullRequest.authorUniqueName))
    .filter((pullRequest) =>
      isAllowedForReview(pullRequest.authorName, pullRequest.authorUniqueName, pullRequest.reviewers ?? [])
    )
...
export async function runSaturn(options: SaturnOptions): Promise<SaturnRunSummary> {
  ...
  if (options.specificPullRequestId !== undefined) {
    ...
    if (requested === undefined) {
      logger.warn(`PR #${String(options.specificPullRequestId)} was not found in the target repo.`);
      candidates = [];
    } else {
      ...
      recordSaturnReview(
        ...
        },
        {
          ...
          comments: (outcome.postedComments ?? []).map((comment) => ({
            ...
            findingId: findingIdOf(comment.filePath, comment.title, comment.category),
            state: 'open'
          })),
          ...
          droppedFindings: (outcome.droppedFindings ?? []).map((finding) => ({
            ...
            findingId: findingIdOf(finding.filePath, finding.title, finding.category),
            state: 'dropped'
          }))
        },
