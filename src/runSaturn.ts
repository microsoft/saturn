...
function selectCandidates(
  activePullRequests: readonly PullRequestSummary[],
...
function summarizeOutcomes(
  ...
  logger.info("");
  logger.info(
    `Done. ${parts.join(", ")}. Inline comments posted: ${String(totalComments)}.`,
  );
}
 ...
 */
export async function runSaturn(
  options: SaturnOptions,
): Promise<SaturnRunSummary> {
  const logger = options.logger ?? consoleLogger;

  let repoRoot = options.repoRoot;
  if (options.managedClone && !options.listOnly) {
    repoRoot = await ensureManagedClone(
      options.cloneDir,
      logger,
      options.updateMaster,
    );
  ...
  if (options.specificPullRequestId !== undefined) {
    const requested = await getPullRequestById(
      repoRoot,
      options.specificPullRequestId,
    );
    if (requested === undefined) {
      logger.warn(
        `PR #${String(options.specificPullRequestId)} was not found in office-bohemia.`,
      );
      candidates = [];
    } else {
      logger.info(
        `Targeting PR #${String(requested.pullRequestId)}: ${requested.title}`,
      );
    ...
    const activePullRequests = await listActivePullRequests(
      repoRoot,
      options.scanLimit,
    );
    ...
    if (options.createdWithinDays !== undefined) {
      const cutoffMs =
        Date.now() - options.createdWithinDays * 24 * 60 * 60 * 1000;
      candidates = candidates.filter(
        (pullRequest) =>
          pullRequest.createdAt !== undefined &&
          Date.parse(pullRequest.createdAt) >= cutoffMs,
      );
    }
    logger.info(
      `Scanned ${String(activePullRequests.length)} active PR(s); ${String(candidates.length)} non-draft candidate(s)` +
        (options.createdWithinDays !== undefined
          ? ` within ${String(options.createdWithinDays)} day(s)`
          : "") +
        ".",
    );
  }
  ...
  if (options.listOnly) {
    for (const pullRequest of candidates) {
      logger.info(
        `  #${String(pullRequest.pullRequestId)} ${pullRequest.title}  (${pullRequest.authorName})`,
      );
    }
  ...
  const allowMcpServerName = options.withAdoMcp
    ? ensureAdoMcpServer(undefined)
    : undefined;
  if (options.withAdoMcp && allowMcpServerName === undefined) {
    logger.warn(
      "Could not register the Azure DevOps MCP server; continuing with the diff fed directly to the model.",
    );
  }

  if (!options.post) {
    logger.info(
      "DRY-RUN: no comments will be posted. Pass --post to publish the review comments.",
    );
  }
  ...
  for (const pullRequest of candidates) {
    ...
    if (
      options.specificPullRequestId === undefined &&
      reviewedCount >= options.maxReviews
    ) {
      break;
    }
